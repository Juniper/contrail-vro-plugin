...
class Utils {
    ...
    fun unparsePortRange(portRange: PortType): String {

    }

    fun unparsePorts(ports: List<PortType>): String {
        return ports.joinToString(",") { unparsePortRange(it) }
    }
    ...
    private fun addressType(
        ...
        log.error(address.subnet?.toString())
        log.error(address.networkPolicy?.toString())
        log.error(address.virtualNetwork?.toString())
        log.error(address.securityGroup?.toString())
        log.error(address.subnetList?.toString())
