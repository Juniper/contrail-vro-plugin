...
fun generateWorkflows(info: ProjectInfo, schema: Schema, config: Config, simpleWorkflows: List<WorkflowDefinition>) {
    createCustomActions(info, schema)
    createComplexWorkflows(simpleWorkflows, info, schema, config)
}

fun generateWorkflowDefinitions(info: ProjectInfo, relations: RelationDefinition, schema: Schema, config: Config): List<WorkflowDefinition> {
