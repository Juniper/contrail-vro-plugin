...
private fun printSummary(scripts: List<File>, tests: List<File>, printer: (String) -> Unit) {
    val scriptMap = scripts.mapById(scriptEndPattern)
    val testMap = tests.mapById(testEndPattern)
...
private fun List<File>.mapById(regex: Regex) =
    associateBy { it.name.replace(regex, "").toLowerCase() }
