...
import static net.juniper.contrail.vro.config.Actions.hasReferenceTo

class HasReferenceToSpec extends WorkflowSpec implements ValidationAsserts{
    def hasReferenceToAction = actionFromScript(hasReferenceTo)
    ...
    def "null child results in success validation"() {
        ...
        def result = engine.invokeFunction(hasReferenceToAction, child, parent)
        then: "it succeeds"
    ...
    def "child which is already referenced by parent results in failure validation with message"() {
        ...
        child.hasVirtualNetworkReference(parent) >> true
        when: "executing validation script"
        def result = engine.invokeFunction(hasReferenceToAction, child, parent)
