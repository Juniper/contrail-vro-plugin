...
class PropertyFormatterSpec extends Specification {
    def basicFormatter = PropertyFormatter.INSTANCE
    ...
    def "Formatting Ipam Subnets class"() {
        ...
        formatted == basicFormatter.format(someSubnetType)
    }
    ...
    def "Formatting PolicyRuleType class"() {
        ...
        def correctlyFormattedPolicyRuleHolder = "${PolicyRuleType.simpleName}: ${basicFormatter.format(obj.policyRuleType)}"

        when: "Formatting to a String"
        def formatted = defaultFormatter.format(obj)

        then: "The resulting string is correct"
        formatted == correctlyFormattedPolicyRuleHolder
