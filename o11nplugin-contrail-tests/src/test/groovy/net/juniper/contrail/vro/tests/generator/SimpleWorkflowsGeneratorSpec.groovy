...
class SimpleWorkflowsGeneratorSpec extends GeneratorSpec{
    ...
    def "expected simple workflows are generated"() {
        ...
        generatedWorkflows.collect { it.displayName }.containsAll(expectedGeneratedWorkflows)
    }
    ...
    def "declared custom workflows are not generated"() {
        ...
        generatedWorkflows.collect { it.displayName }.containsAll(expectedGeneratedWorkflows)
        and:
        generatedWorkflows.collect { it.displayName }.intersect(notExpectedGeneratedWorkflows).size() == 0
    }
}
