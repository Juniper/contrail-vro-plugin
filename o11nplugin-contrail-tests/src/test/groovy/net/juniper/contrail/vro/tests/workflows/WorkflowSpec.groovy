...
//        def conRepoIWantToBind = new OneConnectionRepository(conn)
//        def gpnh = Mock(GlobalPluginNotificationHandler)

        def baseContext = new AnnotationConfigApplicationContext(WorkflowTestConfig.class)

        def configLocations = ["classpath:net/juniper/contrail/vro/plugin.xml"] as String[]
