...
class ComplexWorkflowBuilder(
    ...
    fun workflowInvokation(itemId: Int, outItemId: Int, workflowName: String, workflowPackage: String = basePackageName, setup: BindAggregator.() -> Unit) {
        ...
        val invokationParameters = prepareWorkflowInvokationParameters(
            inputParams,
            outputParams,
            inputMapping,
            outputMapping,
        ...
        val newAttributes = invokationParameters.newAttributes
        val workflowInputBinding = invokationParameters.workflowInputBinding
        val workflowOutputBinding = invokationParameters.workflowOutputBinding

        attributes.addAll(newAttributes)

        val presentation = definition.presentation.bindAttributes(inputMapping)

        val workflowItemId = if (outputParams.any { !outputMapping.containsKey(it.name) }) {
            val nextId = nextId()
    ...
    private fun prepareWorkflowInvokationParameters(
        ...
        return WorkflowInvokationParameters(
            workflowInputBinding,
            workflowOutputBinding,
